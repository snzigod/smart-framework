package com.sn.smart.core.paginator;
...
import com.sn.smart.core.paginator.dialect.Dialect;
import com.sn.smart.core.paginator.domain.PageBounds;
import com.sn.smart.core.paginator.domain.PageList;
import com.sn.smart.core.paginator.domain.Paginator;
import com.sn.smart.core.paginator.support.PropertiesHelper;
import com.sn.smart.core.paginator.support.SQLHelp;

/**
 * 为MyBatis提供基于方言(Dialect)的分页查询的插件
 * 
 * 将拦截Executor.query()方法实现分页方言
 */
			...
			Callable<Paginator> countTask = new Callable() {
				public Object call() throws Exception {
					...
					if (cache != null && ms.isUseCache()
						...
						if (count == null) {
							count = SQLHelp.getCount(ms,
									executor.getTransaction(), parameter,
									boundSql, dialect);
							cache.putObject(cacheKey, count);
						}
					} else {
						count = SQLHelp.getCount(ms, executor.getTransaction(),
