package com.sn.smart.core.paginator.support;
...
public class SQLHelp {
	...
	public static int getCount(final MappedStatement mappedStatement,
		...
		logger.debug("Total count SQL : {"
				+ new SQLFormatter().format(count_sql) + "}]");
		...
		logger.debug("Use Time: {" + (System.currentTimeMillis() - time)
				+ "} Milliseconds");
		return count;
	}
}